...
const ProjectList = dynamic(() => import('@/app/(frontend)/components/Project/ProjectPageList'));
...
export const generateMetadata = async ({
  ...
  const HomeMeta = await fetch(
    `${process.env.API_HOST}meta/${PAGES.projects}`,
    { cache: "no-store" }
  )
    .then((res) => res.json())
    .catch((err) => {
...
const Projects = ({ params }) => {
  return (
    <ProjectList params={params}></ProjectList>
  );
};
